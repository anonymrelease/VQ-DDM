...
class DiscreteDiffusion(nn.Module):
    ...
    def predict_start(self, log_x_t, t, model_kwargs=None):
        ...
        out = self._nn(t, x_t, **model_kwargs)
    ...
    def interpolate(self, x1, x2, t = None, lam = 0.1):
        ...
        xt1_idx, xt2_idx = map(lambda x: x.argmax(1), (xt1,xt2))
        # p = torch.bernoulli(xt1_idx,lam)
        p1 = torch.zeros(256).scatter_(0, torch.tensor(random.sample(range(256),int(lam*256))),1).reshape(1,16,16) if lam!=0 else torch.zeros(256).reshape(1,16,16)
        p2 = torch.zeros(256).scatter_(0, torch.tensor(random.sample(range(256),int(lam*256))),1).reshape(1,16,16) if lam!=0 else torch.zeros(256).reshape(1,16,16)
        p3 = torch.zeros(256).scatter_(0, torch.tensor(random.sample(range(256),int(lam*256))),1).reshape(1,16,16) if lam!=0 else torch.zeros(256).reshape(1,16,16)
        p = torch.stack((p1,p2,p3),0).long().cuda()
        x1_p = p * xt1_idx
        x2_p = (1-p) * xt2_idx
        img = index_to_log_onehot(x1_p + x2_p, self.num_classes)
    ...
    def compute_Lt(self, log_x_start, log_x_t, t, detach_mean=False, model_kwargs=None):
        if model_kwargs is None:
            model_kwargs = {}
    ...
    def _train_loss(self, x, model_kwargs=None):
        ...
        kl = self.compute_Lt(
            log_x_start, self.q_sample(log_x_start=log_x_start, t=t), t, model_kwargs=model_kwargs)
    ...
    def log_prob(self, x, model_kwargs):
        if model_kwargs is None:
            model_kwargs = {}
        b, device = x.size(0), x.device
        if self.training:
    ...
    def sample_chain(self, num_samples, model_kwargs):
        b = num_samples
        device = self.log_alpha.device

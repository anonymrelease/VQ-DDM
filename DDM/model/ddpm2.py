...
from torch import logaddexp as log_add_exp
...
#     return torch.add(maximum , torch.log(torch.exp(a - maximum) + torch.exp(b - maximum)))
...
class DiscreteDiffusionX(torch.nn.Module):
    ...
    def predict_start(self, log_x_t, t):
        ...
        one_hot = F.one_hot(x_t).permute(0,4,1,2,3)
        x_t = 2*x_t / (self.num_classes-1) -1 
        out = self._denoise_fn(t, x_t) 
        out_ = out + one_hot

        assert out_.size(0) == x_t.size(0)
        assert out_.size(1) == self.num_classes
        assert out_.size()[2:] == x_t.size()[1:]
        log_pred = F.log_softmax(out_, dim=1)
        return log_pred
